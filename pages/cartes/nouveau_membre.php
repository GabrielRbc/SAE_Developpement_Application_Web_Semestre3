...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($erreurs)) {
        ...
        if ($retour === true) {
            header("Location: gestion_membre.php?success=1");
            exit();
        } else {
            $erreurs[] = $retour;
        }
...
<nav class="navbar shadow-sm py-3 px-4"
    ...
    <div class="container-fluid d-flex align-items-center">
        <div class="me-3">
            <a href="gestion_membres.php" class="me-2 d-flex align-items-center text-decoration-none text-dark">
                <i class="fa-solid fa-arrow-left menu-item"></i>
            </a>
        </div>

        <div class="flex-grow-1">
            <h5 class="m-0 fw-bold">Nouveau Membre</h5>
        </div>
    </div>
</nav>
        ...
        <form method="post" class="row g-4" id="formNouvelAdherent">
            ...
            <div class="d-flex justify-content-end">
                <button type="submit" class="btn btn-primary d-flex align-items-center gap-2"
                    ...
                    <i class="fa-solid fa-check"></i> Créer le membre
                </button>
            </div>
        </form>
    </div>
